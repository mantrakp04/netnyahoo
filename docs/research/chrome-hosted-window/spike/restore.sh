...
rm -rf rs
APP=$APP "$here/launch.sh" rs 9537 >/dev/null
"$here/dev.sh" rs 'const s=nn.store.getState(); const work=s.createProfile({name:"Work",color:"blue"}); nn.actions.openUrls(["'"${PAGES:-http://localhost:8795}"'/page.html?rs1"]); nn.actions.openWindow({profileId: work}); return work' >/dev/null
sleep 3
"$here/dev.sh" rs 'const s=nn.store.getState(); const ids=Object.keys(s.windows); s.newTab(ids[1],{url:"'"${PAGES:-http://localhost:8795}"'/find.html?rs2"}); return ids' >/dev/null
sleep 3
...
ghosts() { "$here/dev.sh" rs 'return globalThis.expo.modules.NetnyahooCEF.chromeWindows().then(g=>g.map(x=>[x.profile.slice(0,8),x.hosting,x.companion,x.window,x.anyTabBrowserId]))'; }
