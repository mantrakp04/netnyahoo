...
final class DiscoveryTests: XCTestCase {
  func testListsInstalledBrowsersWithProfiles() throws {
    ...
    XCTAssertEqual(list.map(\.id), ["chrome", "arc", "safari", "firefox", "brave", "opera"])
    ...
    XCTAssertEqual(list[5].profiles.map(\.id), ["."])
    XCTAssertEqual(list[5].profiles[0].available, [.bookmarks])
  }
