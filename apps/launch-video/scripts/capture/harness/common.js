...
const log = []; let seq = 0;
const snap = (dir, label) => { const f = `${L}/steps/${dir}/${String(seq++).padStart(4, "0")}.png`; const t = Date.now(); return nn.shell.devSnapshotWindow(W, f).then(() => { log.push({ f, t, t1: Date.now(), label }); }); };
